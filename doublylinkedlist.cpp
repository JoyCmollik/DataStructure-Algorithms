...
public:
 ...
 void addAtTail(int val)
 {
  ...
  newNode->prev = tail; // new making tail prev
 ...
 void addAtIndex(int index, int val)
 {
  ...
  for (int i = 1; i <= index; i++) // if i = 1 executes temp points indx[1],weneed index 2 so that index 2 next points our new node
  {
   temp = temp->next;
  }
 ...
 void deleteAtIndex(int index)
 {
  ...
  else if (index == size - 1) // if last node selected
  {
   ...
   tail->prev = NULL; // tail now becomes disconnected
